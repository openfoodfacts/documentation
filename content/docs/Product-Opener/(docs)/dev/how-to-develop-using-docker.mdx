...
This page describes how to test and debug your changes once you have set up the project, Product Opener with Docker using [dev environment quick start guide](how-to-quick-start-guide/).
...
``` sh
make log
```text title="Tail Docker Compose logs"
...
```text title="Opening a shell in a Docker container"
...
```text title="Running minion jobs"
...
```text title="Run queries on MongoDB database"
